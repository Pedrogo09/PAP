...
def order_list(request):
    ...
    return render(request, 'bar_app/order_list.html', {'orders': orders})
    ...
    if order.can_be_cancelled():
        ...
        if order.payment_method == 'card':
            Transaction.objects.create(
                user=request.user, transaction_type='refund', amount=order.total_amount,
                order=order, description=f'Reembolso pedido {order.order_number}'
            )
...
def profile(request):
    ...
    context = {'recent_orders': recent_orders, 'recent_transactions': recent_transactions}
        ...
        if 'payment_method' in request.POST:
            ...
            try:
                ...
                SchoolTransaction.objects.create(
                    ...
                    description=f'Carregamento do utilizador {request.user.username}', created_by=request.user
                )
            except Exception:
                pass
