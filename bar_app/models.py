...
class Product(models.Model):
    ...
    def get_user_allergens_conflict(self, user):
        """Retorna os alergénios do produto que colidem com os do utilizador"""
        if not user or not user.is_authenticated or not user.allergens:
            return []
        
        user_list = [a.strip().lower() for a in user.allergens.split(',') if a.strip()]
        if not user_list:
            return []
            
        if not self.allergens:
            return []
        product_list = [p.strip().lower() for p in self.allergens.split(',') if p.strip()]
        
        conflicts = []
        for u in user_list:
            for p in product_list:
                if u in p or p in u:
                    conflicts.append(p.title())
...
class Order(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Enviar email quando o estado muda para 'ready' (Pronto)
        if not is_new and old_status != 'ready' and self.status == 'ready':
            self.send_ready_email()

    def send_ready_email(self):
        from django.core.mail import send_mail
        try:
            subject = f"O teu pedido {self.order_number} está pronto!"
            message = (
                f"Olá {self.user.get_full_name() or self.user.username},\n\n"
                f"Temos boas notícias! O teu pedido {self.order_number} está pronto para levantamento no bar da escola.\n\n"
                f"Detalhes do Pedido:\n"
            )
            for item in self.items.all():
                message += f"- {item.quantity}x {item.product.name}\n"
            
            time_str = self.scheduled_time.strftime('%H:%M') if self.scheduled_time else ''
            message += (
                f"\nTotal: €{self.total_amount}\n"
                f"Hora Agendada: {time_str}\n\n"
                f"Por favor, dirige-te ao balcão para levantar o teu pedido.\n\n"
                f"Bom apetite!\nEquipa do Bar Escolar"
            )
            send_mail(
                subject,
                message,
                None,  # Usa DEFAULT_FROM_EMAIL definido no settings.py
                [self.user.email],
                fail_silently=True
            )
        except Exception:
...
class StockMovement(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Erro ao processar transação contabilística de stock: {e}")
